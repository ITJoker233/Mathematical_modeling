...
class Excel:
    ...
    def _dict_data(self,start,end,titles):
        all_data = {}
    ...
    def dict_data(self):
        ...
        threads = []
        threadNum = 2
        if self.sh.max_row % 2 == 0:
            slices = self.sh.max_row / 2
        else:
            slices = (self.sh.max_row-1) / 2
        while not int(slices) % 2:
            slices = slices / 2
            threadNum += 2
        for x in range(threadNum):
            end += slices
            threads.append(Thread(self._dict_data, args=(start, end,titles)))
            threads[x].start()
            threads[x].join()
            start = end
        for thread in threads:
            while thread.isAlive():
                pass
            all_data.append(threands[x].result)
